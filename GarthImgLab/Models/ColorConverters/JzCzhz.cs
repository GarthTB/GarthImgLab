...
public readonly struct JzCzhz: IColorSpace<JzCzhz> {
    ...
    private static readonly double[] Cusps = CuspLut.Build<JzCzhz>(1, FromLinearSRgb(0, 0, 1).C);
    ...
    public static (double L, double C, double H) FromLinearSRgb(double r, double g, double b) {
        var (x, y, z) = Xyz.FromLinearSRgb(r, g, b);
    ...
    public static (double R, double G, double B) ToLinearSRgb(double j, double c, double h) {
        var hzRad = h * (PI / 180);
        ...
        return Xyz.ToLinearSRgb(x, y, zp);
    }
}
