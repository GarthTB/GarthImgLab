...
public static class CalcMaxSat {
    private const int Max = (1 << 12) - 1;
    private const double InvMax = 1d / Max;

    public static void Run() {
        var max = Enumerable.Repeat((.0, .0, .0, S: double.NegativeInfinity), 14).ToArray();
        ...
        Parallel.ForEach(
            ...
            local => {
                lock (max)
                    for (var i = 0; i < 14; i++)
                        if (local[i].S > max[i].S)
                            max[i] = local[i];
            });

        foreach (var (r, g, b, s) in max) Console.WriteLine($"({r},{g},{b})\t{s}");
    }
}
